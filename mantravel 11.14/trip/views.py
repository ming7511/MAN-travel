...
import re
from third_service.kimi_api import multi_turn_chat,simplechat
...
def delete_trip(request, trip_id):
    try:
        trip = TripInformation.objects.get(id=trip_id, user_id=request.user.id)
        trip.delete()
        return JsonResponse({"message": "行程删除成功"}, status=204)
    ...
    for day, destinations in itinerary.items():
        for order, destination in enumerate(destinations, start=1):
            ...
            TripPlan.objects.create(
                ...
                days=day,
                trip_destination=destination,  # 景点或活动名称
                trip_order=order,  # 顺序
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_nameAndlocation(request,trip_id):
    """
    视图函数：根据行程ID和景点描述获取景点的准确名字和经纬度
    请求参数:
    ...
    trip_description = trip_plan.trip_destination
    ...
    # 调用 geocode 获取经纬度
    geocode_result = amap_client.geocode(address=poi_name, city=trip_name)

    if geocode_result:
        longitude, latitude, _ = geocode_result  # 提取经纬度
        location = f"{longitude},{latitude}"  # 转换为字符串形式
        trip_plan.location = location  # 存储到数据库
        trip_plan.save()  # 保存更新到数据库
    else:
        return JsonResponse({"error": "未能获取地点的经纬度"}, status=404)
    ...
    return JsonResponse({
        ...
        "poi": {
            "name": poi_name,
            "location": trip_plan.location  # 返回存储的经纬度
        }
    })
...
def get_photo(request,trip_id):
    ...
    first_poi = pois[0]
    poi_name = first_poi.get("photos")
    name = first_poi.get("name")
    # 获取第一张图片
    photo = poi_name[0].get("url")
    # 将url存入数据表TripPlan的photo字段
    trip_plan.photo = photo
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def recommend_hot_spots(request):
    """
    视图函数：根据城市名称推荐热门景点，返回景点的名称、标签和描述
    请求参数:
    - city: 城市名称
    """
    ...
    if not city:
        return JsonResponse({"error": "缺少必要的参数 city"}, status=400)

    # 构造查询内容
    query = f"我想要去{city}玩，请你罗列出热门景点"

    # 调用 simplechat 函数获取响应
    response = simplechat(query)

    # 打印响应内容，查看返回值
    # print("Response from simplechat:", response)

    # 将响应数据按预期格式进行处理
    positions = []
    try:
        # 去除 `<answer>` 和 `</answer>` 部分
        cleaned_response = response.strip().replace("<answer>", "").replace("</answer>", "").strip()

        # 分割每一个景点信息
        places = cleaned_response.split("\n")

        for place in places:
            # 确保每个景点信息符合我们期望的格式
            print(place)
            if "(" in place and ")" in place and "：" in place:
                name_tag_desc = place.split("(")
                name = name_tag_desc[0].strip()
                tag_desc = name_tag_desc[1].split(")：")
                tag = tag_desc[0].strip()
                description = tag_desc[1].strip()
        ...
        return JsonResponse({"error": "数据解析失败", "details": str(e)}, status=500)
