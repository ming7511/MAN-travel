...
SIMPLE_JWT = {
    'ACCESS_TOKEN_LIFETIME': timedelta(days=3650),  # 可根据需要调整
...
CACHES = {
    'default': {
        ...
        'OPTIONS': {
            'CLIENT_CLASS': 'django_redis.client.DefaultClient',  # 使用默认的 Redis 客户端类
            'PASSWORD': '160127ss',
            'CONNECTION_POOL_CLASS': 'redis.connection.BlockingConnectionPool',  # 使用连接池
