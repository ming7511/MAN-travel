<template> 
  <div class="bottom-nav">
    <!-- 行程计划 -->
    <div class="nav-item" @click="navigateTo('plans')">
      <image class="icon" src="/static/icons/add-p.png" alt="行程计划图标" />
      <span>行程计划</span>
    </div>
    
    <!-- 添加行程 -->
    <div class="nav-item add-trip" @click="addNewTrip">
      <span class="add-icon">+</span>
    </div>
    
    <!-- 我的 -->
    <div class="nav-item" @click="navigateTo('profile')">
      <image class="icon" src="/static/icons/user.png" alt="我的图标" />
      <span>我的</span>
    </div>
  </div>
</template>
...
const addNewTrip = () => {
  uni.redirectTo({
    url: '/pages/create_method/create_method'
  });
};
</script>
...
.icon {
  width: 24px; /* 图标大小 */
  height: 24px;
  margin-bottom: 5px; /* 为文本与图标留出一定距离 */
}
