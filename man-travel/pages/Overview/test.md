	...
	export default {
		...
		methods: {
			...
			initTripData(trip) {
				...
				// 初始化行程相关数据
				this.tripTitle = trip.trip_name || ''; // 行程标题
				this.travelDateRange = `${trip.start_date} 至 ${trip.end_date}`; // 旅行日期范围

				// 计算旅行时长
				const startDate = new Date(trip.start_date);
				const endDate = new Date(trip.end_date);
				const timeDiff = endDate - startDate;
				const daysDiff = timeDiff / (1000 * 3600 * 24) + 1; // 加1是因为开始和结束日期都包含
				this.tripDuration = `${daysDiff}天`;

				// 其他行程信息
				this.places = trip.places || [];
				this.weatherForecast = trip.weather || [];
				this.dailyTrips = trip.dailyTrips || [];
				this.placeCoordinates = trip.placeCoordinates || {};

				// 根据 duration 动态生成天数按钮
				const dayCount = daysDiff;
				this.days = ['总览', ...Array.from({
					length: dayCount
				}, (_, i) => `DAY${i + 1}`)];
							...
							if (coordinates) {
								...
								if (prevMarker) {
									...
									const polyline = new AMap.Polyline({
										path: [prevMarker.getPosition(), marker
											.getPosition()
										],
										map: that.map,
