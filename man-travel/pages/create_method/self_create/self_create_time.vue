  ...
  uni.request({
    ...
    success: (res) => {
      ...
      if (res.statusCode === 201 || res.statusCode === 200) { // 根据后端返回的状态码调整
        ...
        const tripId = res.data.trip_id || res.data.id || res.data.tripId; // 根据实际响应调整
        ...
        if (tripId) {
          ...
          // 将 dataToUpload 转为 JSON 字符串，然后编码
          const dataString = encodeURIComponent(JSON.stringify(dataToUpload));
    
          // 跳转到 Overview 页面，并传递 trip_id 以及其他数据
          uni.navigateTo({
            url: `/pages/Overview/Overview?trip_id=${tripId}&data=${dataString}` // 传递 trip_id 和其他数据
          });
        } else {
          // trip_id 不存在
          uni.showToast({
            title: '行程创建成功，但未获取到 trip_id',
            icon: 'none',
          });
          console.error('trip_id 未在响应中找到:', res.data);
        }
      } else {
        // 如果服务器返回的是非成功状态码
        uni.showToast({
